	...
	case H10_KEY_MANUAL:
		...
		else
		{
          ...
          case H10_KEY_MUSIC:
            ...
            break;
          case H10_KEY_RUBBING:
            nMaunalSubMode = nMaunalSubMode_RUBBING;
            break;
          case H10_KEY_MANUAL:
            nMaunalSubMode++;
	    if(nMaunalSubMode > 7)
          ...
          case nMaunalSubMode_MUSIC:
            ...
            if((nKeyBackLocate == LOCATE_NONE) || (nKeyBackLocate == LOCATE_FULL_BACK))//修改韵律到顶可能不下行的BUG  dyl 20171222
            {
              nKeyBackLocate = LOCATE_FULL_BACK ;
              /*
              if((nCurWalkMotorState == STATE_IDLE) ||
            ...
            nMaxActionStep = 2;//修改韵律到顶可能不下行的BUG  dyl 20171222//4 ;
            nStartActionStep = 0 ;
            bBackManualModeInit = TRUE ;
            //开启振动
            // bKeySeatVibrate = TRUE ;
        ...
        if((bKneckCheckSwitchLast != BODY_TOUCHED) && (bKneckCheckPosition == BODY_TOUCHED))
        {
            ...
            if(currentPosition > LIMIT_POSITION && currentPosition <= TOP_POSITION)
            {
              ...
              /********************fww*******************
                    bodyDetectSuccess = 1;
                    //上移,找到实际肩部位置
                    nShoulderPosition = currentPosition + 25;
                    nFinalWalkMotorLocate = currentPosition;
                    //
                    nWalkMotorControlParam1 = WALK_LOCATE_SHOULDER ;
                    nWalkMotorControlParam2 = nShoulderPosition ;
                    bWalkMotorInProcess = TRUE ;
                    bUpdateLocate = TRUE ;
                    //fww 删掉
                    BodyDataRefresh();
