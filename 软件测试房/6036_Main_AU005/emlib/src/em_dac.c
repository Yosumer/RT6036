...
//20140414
void DAC_Set_Cal(DAC_TypeDef *dac,unsigned int ch, unsigned int data)
{
  if (ch == 0)
  {
    dac->CAL = data;
...
unsigned int DAC_Get_Cal(DAC_TypeDef *dac,unsigned int ch)
{
  if (ch == 0)
  {
    return(dac->CAL);
...
void DAC_Set_Data(DAC_TypeDef *dac,unsigned int ch, unsigned int data)
{
  if (ch == 0)
  {
    dac->CH0DATA = data;
...
unsigned int DAC_Get_Data(DAC_TypeDef *dac,unsigned int ch)
{
  if (ch == 0)
  {
    return(dac->CH0DATA);
